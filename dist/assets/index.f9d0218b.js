...
function pf(t, ...e) {
  ...
  return function(t2, e2, n2, s2) {
    ...
    return t2.asyncQueue.enqueueAndForget(async () => Vc(await Ka(t2), r3)), () => {
      i2.Sc(), t2.asyncQueue.enqueueAndForget(async () => Sc(await Ka(t2), r3));
    };
  }(Ph(a), h, u, c);
}
function Tf(t, e) {
...
function post(inputShowModal) {
  ...
  if (user) {
    ...
    const id2 = user.uid;
    const document2 = yf(_h(db, "Publicaciones"), {
...
function getPost(callBack) {
  ...
  pf(consulta, callBack);
}
const deletePosta = (id2) => gf(mh(db, "Publicaciones", id2));
async function editPost(postId, contenido) {
  const postRef = mh(db, "Publicaciones", postId);
  await mf(postRef, {
    contenido
  });
}
